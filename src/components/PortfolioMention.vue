<template>
	<section :id="stringToSlug(data.title)" :style="style" :class="{'read-more-active': modalActive}"
			 class="portfolio-mention">
		<div class="content-wrapper">
			<div class="content">
				<div class="text-wrapper">
					<div class="header">
						<project-tag v-for="(tag, index) in data.tags" :key="index" :message="tag"/>
						<a :href="'#' + stringToSlug(data.title)" style="text-decoration: none"
						   @click="() => onClickTitle(stringToSlug(data.title))">
							<LinkHeader><h2>{{ data.title }}</h2></LinkHeader>
						</a>
					</div>
